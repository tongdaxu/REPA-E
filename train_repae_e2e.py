...
def main(args):    
    ...
    kl_weight = loss_cfg["losses"]["kl_weight"]
    loss_cfg["losses"]["kl_weight"] = 0
    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.accumulate([model, vae, vae_loss_fn]), accelerator.autocast():
                ...
                # loss = DKL = E[-(-log q(z|x)) + J_DSM(g(.)^2)]
                #      = -h(q(z|x)) + E[J_DSM(g(.)^2)]
                # sgvba_loss = vae_align_outputs["wsm_loss"] - torch.mean(posterior.nll(z))
                sgvba_loss = vae_align_outputs["wsm_loss"] - torch.mean(torch.sum(0.5 * posterior.logvar, dim=(1,2,3)))
                sgvbb_loss = torch.mean(posterior.kl())
                sgvba_alpha = global_step / args.max_train_steps
                sgvb_loss = sgvba_alpha * sgvba_loss + (1-sgvba_alpha) * sgvbb_loss
                vae_loss = vae_loss + args.vae_align_proj_coeff * vae_align_outputs["proj_loss"].mean() + kl_weight * sgvb_loss
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    "sgvba_alpha": sgvba_alpha,
                    "sgvba_loss": accelerator.gather(sgvba_loss).mean().detach().item(),
